...
export function AboutPage() {
  ...
  const currentPlatformTarget = detectCurrentPlatformTarget(platform.metadata.isTauri);
    ...
    <>
      ...
      <div className="max-w-md mx-auto h-full flex items-center">
        <div className="flex flex-col items-center text-center space-y-5">
          ...
          <FadeIn delay={280}>
            <div className="w-full max-w-xl rounded-xl border border-border/60 bg-muted/20 p-4 text-left">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <h2 className="text-sm font-medium">Support policy</h2>
                  <p className="text-xs text-muted-foreground">
                    Core engines and tier-1 platforms get the most validation. Everything else is
                    best-effort until proven boring.
                  </p>
                </div>
                <div className="rounded-full border border-border/60 px-2.5 py-1 text-[11px] text-muted-foreground">
                  Current target: {currentPlatformTarget.label} ·{' '}
                  {PLATFORM_TIER_LABELS[currentPlatformTarget.tier]}
                </div>
              </div>

              <div className="space-y-2">
                {PLATFORM_SUPPORT_TARGETS.map((target) => (
                  <div
                    key={target.id}
                    className="flex items-start justify-between gap-3 rounded-lg border border-border/40 bg-background/60 px-3 py-2"
                  >
                    <div>
                      <div className="text-sm font-medium">{target.label}</div>
                      <div className="text-xs text-muted-foreground">{target.notes}</div>
                    </div>
                    <div className="rounded-full px-2 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">
                      {PLATFORM_TIER_LABELS[target.tier]}
                    </div>
                  </div>
                ))}
              </div>
